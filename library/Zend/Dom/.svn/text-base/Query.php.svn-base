...
class Zend_Dom_Query
{
    ...
    public function queryXpath($xpathQuery, $query = null)
    {
		...
		$encod  = mb_detect_encoding($document);
		$headpos = mb_strpos($document,'<head>');
		if (FALSE=== $headpos)
			$headpos = mb_strpos($document,'<HEAD>');
                if (FALSE!== $headpos) {
                        $headpos += 6;
                        $content = mb_substr($document,0,$headpos) . 
				'<meta http-equiv="Content-Type" content="text/html; charset='.$encod.'">' .
				mb_substr($document,$headpos);
		}
                $document=mb_convert_encoding($document, 'HTML-ENTITIES', $encod);
